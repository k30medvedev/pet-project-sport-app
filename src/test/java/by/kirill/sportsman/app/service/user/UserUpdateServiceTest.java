...
class UserUpdateServiceTest {
    ...
    void shouldUpdateUserName() {
        ...
        long id = 227L;
        Mockito.when(updateReq.getId()).thenReturn(id);
        Mockito.when(updateReq.getFirstName()).thenReturn("Jonn");
        Mockito.when(updateReq.getLastName()).thenReturn("Milton");
        Mockito.when(updateReq.getBirthday()).thenReturn(birthday);
        Mockito.when(updateReq.getEmail()).thenReturn("222@tut.by");

        Mockito.when(userSearchService.findById(id)).thenReturn(expectedSearchResult);

        Mockito.when(userRepository.save(expectedSearchResult)).thenReturn(expectedUpdateResult);
        //WHEN
